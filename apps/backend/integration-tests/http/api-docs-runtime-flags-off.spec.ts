...
async function bootstrapAdminBearerToken(
  ...
  const email = "api-docs-runtime-off@medusa.test"
...
if (!requestedDatabaseName) {
  describe("API docs runtime flags-off HTTP integration", () => {
  ...
  medusaIntegrationTestRunner({
    ...
    testSuite: ({ api, getContainer }) => {
      ...
      describe("flags disabled", () => {
        it.each([
          "/openapi/store.json",
          "/openapi/admin.json",
          "/openapi/webhooks.json",
          "/docs",
        ])("returns opaque 404 anonymously for %s", async (routePath) => {
        ...
        it.each(["/openapi/admin.json", "/openapi/webhooks.json"])(
          "returns opaque 404 with bearer auth for %s",
          async (routePath) => {
            const response = await api.get(routePath, {
