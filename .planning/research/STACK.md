...
**Domain:** Headless Print-on-Demand (POD) t-shirt e-commerce **backend** for Brazil (Medusa v2)
**Researched:** 2026-06-22
**Confidence:** HIGH (core platform + payments verified against official docs / Context7); MEDIUM on a few exact patch versions (Resend) and Pix-via-Medusa wiring (needs phase research)

> Scope note: This is a **backend-only** milestone. No storefront/UI frameworks are recommended here — only what the backend needs to expose stable API contracts for a future storefront. The Medusa Admin dashboard ships *with* the backend and is in scope; standalone frontend tooling is not.
...
npx create-medusa-app@latest pod-backend
...
# 3) Integrations
npm install stripe@^19            # Stripe Node SDK (server-side)
npm install resend@^4             # Transactional email
npm install posthog-node@^5       # Server-side analytics (purchase_completed outbox)
npm install @sentry/node@^10      # Error monitoring
...
npm install axios            # optional; Node 22 fetch works too

# 6) Dev / process / proxy (system-level)
npm install -g pm2@^6        # on the VPS
# Nginx + Certbot installed via the OS package manager (apt)
```

`medusa-config.ts` — production module wiring (shape):

```ts
module.exports = defineConfig({
  projectConfig: {
    databaseUrl: process.env.DATABASE_URL,        // Supabase pooled connection string
    redisUrl: process.env.REDIS_URL,
    workerMode: process.env.MEDUSA_WORKER_MODE,    // "server" | "worker" | "shared"
    http: { /* storeCors, adminCors, jwtSecret, cookieSecret */ },
  },
  modules: [
    { resolve: "@medusajs/medusa/cache-redis", options: { redisUrl: process.env.CACHE_REDIS_URL } },
    { resolve: "@medusajs/medusa/event-bus-redis", options: { redisUrl: process.env.EVENTS_REDIS_URL } },
    { resolve: "@medusajs/medusa/workflow-engine-redis", options: { redis: { url: process.env.WE_REDIS_URL } } },
    {
      ...
      options: { providers: [{
        ...
        options: {
          ...
          automatic_payment_methods: true,   // surfaces Pix + card (BRL) from Stripe Dashboard
          capture: true,
        },
      }] },
    },
    {
      ...
      options: { /* Supabase Storage S3 endpoint, bucket, region, access keys */ },
    },
    // + custom modules: gelato-fulfillment, payment-attempt, webhook-log, etc.
  ],
})
```
...
These are **not** optional patterns — they define how each capability is built in v2:
...
**If running on a single small VPS (MVP default):**
...
**If Stripe Pix pricing/availability becomes a blocker in Brazil:**
...
---
*Stack research for: Brazilian POD t-shirt e-commerce backend on Medusa v2*
*Researched: 2026-06-22*
