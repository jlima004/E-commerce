...
### 2.19 Fundação Store — idempotência transversal e resource version

A Phase 13 introduz duas persistências foundation distintas do nascimento de `Order`:

- `StoreIdempotencyRecord` — primitive PostgreSQL transversal de claim/replay/conflito para operações Store repetíveis. **Não** reutiliza `CheckoutCompletionLog`.
- `StoreResourceVersion` — contador monotônico server-authoritative por `(resource_type, resource_id)` para optimistic concurrency. O contrato documental entra em v1.22; a implementação runtime/módulo permanece no plano `13-05`.

Premissas vinculantes:

- PostgreSQL é a única fonte de verdade de claim, fingerprint, lifecycle e CAS.
- Redis pode coordenar contenção curta, mas nunca decide correctness.
- `Idempotency-Key` identifica retry; nunca prova ator, ownership, capability, autenticação ou autorização.
- Escopo único: `UNIQUE(operation, actor_scope_hash, resource_scope_hash, idempotency_key_hash)`.
- Hash da key: HMAC-SHA-256 com `STORE_IDEMPOTENCY_KEY_PEPPER` dedicado; persistir apenas `idempotency_key_hash`, `hash_version=hmac-sha256-v1` e `pepper_version=1`.
- Nunca persistir raw Idempotency-Key, pepper, JWT, capability, token puro, PII sensível, `client_secret`, material Pix ou provider payload.
- Lifecycle finito: nenhum estado não terminal sem deadline de progressão; todo terminal com `expires_at` finito.
- Driver factual: Medusa scheduled job `store-idempotency-lifecycle` com cron `* * * * *`.
- `CheckoutCompletionLog` permanece exclusivo da criação idempotente de `Order` pós-webhook Stripe.
...
| `StoreIdempotencyRecord` | Custom | `id`, `operation`, `actor_scope_hash`, `resource_scope_hash`, `idempotency_key_hash`, `hash_version`, `pepper_version`, `request_fingerprint`, `state`, `state_version`, result allowlisted, deadlines, `expires_at` | Primitive transversal Store de claim/replay/conflito. Escopo composto; PostgreSQL decide o único claim. |
...
### 4.17 StoreIdempotencyRecord — Idempotência Store Transversal

Entidade customizada responsável pelo primitive PostgreSQL de claim, replay e conflito para operações Store repetíveis.

#### Finalidade

- Garantir exatamente um claim ativo por `(operation, actor_scope_hash, resource_scope_hash, idempotency_key_hash)`.
- Reutilizar resultado seguro quando a mesma key e a mesma intenção (`request_fingerprint`) forem observadas.
- Rejeitar same-key/different-intent sem side effect e sem transferência de ownership.
- Progressar lifecycle finito via driver scheduled mesmo sem nova request HTTP.
- Minimizar dados: hash da key + fingerprint + resultado allowlisted; nunca plaintext sensível.
...
```json
{
  ...
  "state": "processing | completed | failed_retryable | failed_terminal | reconciliation_required | reconciliation_unresolved",
  "state_version": 1,
  "result_type": "string | null",
  "result_id": "string | null",
  "response_status": "number | null",
  "result_safe_metadata": "object | null",
  "locked_at": "datetime | null",
  "state_deadline_at": "datetime | null",
  "next_retry_at": "datetime | null",
...
- Primary key: `id`
- Unique claim: `UNIQUE(operation, actor_scope_hash, resource_scope_hash, idempotency_key_hash)`
- CHECK `state` ∈ estados listados
- CHECK `hash_version = 'hmac-sha256-v1'` (versão corrente)
- CHECK `pepper_version = 1` (versão corrente)
- CHECK `state_version >= 1`
- Index: `(state, state_deadline_at)`
- Index: `(next_retry_at)`
- Index: `(expires_at)`

#### Idempotency-Key e hashing

- Input: bytes US-ASCII visíveis `[!-~]`, comprimento **1..255 bytes**, byte-for-byte (sem trim/case-fold/normalização Unicode).
- Algoritmo: HMAC-SHA-256 via `node:crypto`.
- Secret: `STORE_IDEMPOTENCY_KEY_PEPPER` (base64url; decoded ≥32 bytes em produção). Proibido reutilizar JWT/COOKIE/Stripe/guest/confirmation secrets.
- Persistidos: `idempotency_key_hash`, `hash_version`, `pepper_version` somente.
- Rotação transparente é proibida neste milestone; troca exige gate humano dual-read.

#### Fingerprint

- `request_fingerprint` deriva de DTO público já validado, serializado de forma canônica (chaves ordenadas; sem campos voláteis como correlation ID/headers).
- Não persistir raw request body como fingerprint.
- Same fingerprint ⇒ same intent; fingerprint diferente no mesmo scope+key ⇒ conflito.

#### Lifecycle / TTL policy map
...
- `phase13.local-mutation` — retention terminal 24h
- `phase13.uncertain-effect-simulation` — review 7d; unresolved retention 30d

#### Segurança (negativas)

Proibido persistir ou logar: raw Idempotency-Key, pepper, JWT, capability, Authorization, cookie, CPF/PII, `client_secret`, Pix QR/copia-e-cola, provider payload.

---

### 4.18 StoreResourceVersion — Optimistic Concurrency (contrato documental)

Entidade customizada para versão monotônica server-authoritative de recursos Store concorrentes.
...
| `DATA-123` | `StoreIdempotencyRecord` deve garantir unicidade de claim por `UNIQUE(operation, actor_scope_hash, resource_scope_hash, idempotency_key_hash)`. |
| `DATA-124` | `StoreIdempotencyRecord` nunca persiste raw Idempotency-Key, pepper, capability, token puro, PII sensível ou provider payload; apenas hash/versão e resultado allowlisted. |
| `DATA-125` | Estados de `StoreIdempotencyRecord` são exatamente `processing`, `completed`, `failed_retryable`, `failed_terminal`, `reconciliation_required`, `reconciliation_unresolved`. |
| `DATA-126` | Todo estado não terminal de `StoreIdempotencyRecord` possui deadline de progressão (`state_deadline_at` e/ou `next_retry_at`); nenhum pode permanecer indefinido. |
| `DATA-127` | Todo estado terminal de `StoreIdempotencyRecord` possui `terminalized_at` e `expires_at` finitos; cleanup remove somente terminais expirados. |
...
| `StoreIdempotencyRecord` | unique `(operation, actor_scope_hash, resource_scope_hash, idempotency_key_hash)` | Claim atômico por escopo+key. |
